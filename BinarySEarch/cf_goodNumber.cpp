...
int32_t main()
{
    ...
    while(t--)
    {
        ...
        {
            fill(all(s), '0');
            cout << power(sz(s)) << endl;

        }
        else
        {
            fill(s.begin() + two_index, s.end(), '0');
            // pr(s);

            s = "0" + s;
            for(int i = two_index; i >= 0; i--)
            {
                s[i] += 1;
                if(s[i] == '2')
                {
            ...
            ll ans=0;
            // revall(s);
            for(int i = 0, j=sz(s)-1; i <sz(s), j>=0; i++,j--)
            {
            	
                ans += (s[i]-'0')*power(j);
            }
            cout << ans << endl;
            // pr(sizeof(t),sizeof(ans),sizeof(s),sizeof(two_index));
        }
