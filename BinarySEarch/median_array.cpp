...
double Solution::findMedianSortedArrays(const vector<int> &va, const vector<int> &vb)
{
    int na  = va.end() - va.begin();
    int nb  = vb.end() - vb.begin();

    int lo = min(va[0], vb[0]);
    int hi = max(va[na - 1], vb[nb - 1]);
